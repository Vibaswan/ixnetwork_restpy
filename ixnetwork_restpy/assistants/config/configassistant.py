...
class ConfigAssistant(object):
    ...
    def commit(self):
        ...
        # print(self._ixnetwork._xpathObj._children)
        importing = time()
        self._ixnetwork._xpathObj._mode = 'normal'
        self._ixnetwork.ResourceManager.ImportConfig(json.dumps(config), False)
        # print('import_config took time ==' + str(time() - importing))
        # start = time()
        self.__get_hrefs()
        # print('getting and filling up time ==' + str(time() - start))
        self.clear_config()
